...
//разделить на классы: result и вычисление арктангенса
...
class Result {
    ...
    public Result(double sum, double[] fraction, double[] sumArray, double predstavleniya_error, double obryv_ryada_error, double okryglenie_error) {
        this.sum = sum;
...
//        System.out.println("Ошибка представления y = " + arctany.predstavleniya_error);
        ...
        arctanx.okryglenie_error *= gamma(nMaxx - 1);
        ...
        arctany.okryglenie_error *= gamma(nMaxx - 1);
...
//        System.out.println("Ошибка суммирования y = " + sum_errory);
        ...
        double atanx16 = 16.0 * arctanx.sum; // (аргтангенс + ошибка) * 16 = 16 * arct + 16 *ошибка  у х на 16, у на 4
        double atany4 = 4.0 * arctany.sum;
        double atanx16MinusAtany4 = atanx16 - atany4;

        double raznost_error = atanx16MinusAtany4*U;

        double total_error = abs_errorx * 16 + abs_errory * 4 + raznost_error; //будет больше потому что сама ошибка с ошибкой округления
        //округление всех ошибок в большую сторону функция в джаве, везде где моожет произойти округление ошибки надо её завысить в большую стоорону
